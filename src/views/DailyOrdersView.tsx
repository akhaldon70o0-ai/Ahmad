...
export const DailyOrdersView: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!customerName.trim() || !phone.trim() || !address.trim() || !details.trim()) {
      return alert('Please fill in customer name, phone, address, and order items.');
    }
  ...
  const printPackingSlips = () => {
    ...
    if (!activeOrders.length) return alert('No orders in Preparation or Ready status to print.');
                  ...
                  <tr key={o.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <button
                          onClick={() => {
                            if (window.confirm(`Delete order for "${o.customerName}"?`)) {
                              deleteDailyOrder(o.id);
                            }
                          }}
