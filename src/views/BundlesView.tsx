...
export const BundlesView: React.FC<BundlesViewProps> = ({ initialItemName }) => {
  ...
  const handleAddItemToBundle = () => {
    ...
    if (!clean) return alert('Select a product to add to the bundle.');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!bundleName.trim() || isNaN(priceNum) || currentItems.length === 0) {
      return alert('Please enter bundle name, selling price, and at least one included product.');
    }
                  ...
                  <tr key={b.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <button
                          onClick={() => {
                            if (window.confirm(`Delete bundle "${b.name}"?`)) {
                              deleteBundle(b.id);
                            }
                          }}
